...
@app.route("/random_smpc_form", methods=["GET"])
def random_smpc_form():
    csv_text = _current_csv_text(required=False)
    if not csv_text:
        return "Please upload a dataset first.", 400
    df = pd.read_csv(StringIO(csv_text))
    numeric_cols = df.select_dtypes(include='number').columns.tolist()
    return render_template("smpc_random_form.html", columns=numeric_cols)
#
# @app.route("/run_random_smpc", methods=["POST"])
# def run_random_smpc():
#     from smpc import simulate_random_smpc
#     column = request.form["column"]
#     csv_text = session["csv_data"]
#
#     try:
#         result = simulate_random_smpc(csv_text, column)
#     except Exception as e:
#         return f"SMPC simulation failed: {e}", 500
#
#     # Pass the result dictionary to the summary page
#     return render_template("smpc_summary.html", result=result)
